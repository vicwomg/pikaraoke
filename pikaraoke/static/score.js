...
function getScoreData(scoreValue){
	if (scoreValue < 30) {
		return {applause: "applause-l.ogg", review: scoreReviews.low[Math.floor(Math.random() * scoreReviews.low.length)]}
	} else if (scoreValue < 60) {
		return {applause: "applause-m.ogg", review: scoreReviews.mid[Math.floor(Math.random() * scoreReviews.mid.length)]}
	} else {
...
async function showFinalScore(scoreTextElement, scoreValue, scoreReviewElement, scoreData){
...
async function rotateScore(scoreTextElement, duration) {
	...
	while (true) {
		...
		const randomScore = String(Math.floor(Math.random() * 99) + 1).padStart(2, '0');
