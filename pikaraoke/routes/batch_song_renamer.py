...
def score_result(result, original_query):
   ...
   # Check if the song name or the artist name is written entirely in uppercase and penalize
    ...
    if part2:
        if (part1_normalized == track_title_normalized and 
            part2_normalized == artist_name_normalized):
            score += 100
            matched = True
        elif (part1_normalized == track_title_normalized or 
              part1_normalized in track_title_normalized):
            score += 50
            matched = True
    
    # Case 2: part1 is artist, part2 is title (ex: "Coldplay - Viva La Vida")
    if part2 and not matched:
        if (part2_normalized == track_title_normalized and 
            part1_normalized == artist_name_normalized):
            score += 100
            matched = True
        elif (part2_normalized == track_title_normalized or 
              part2_normalized in track_title_normalized):
            score += 50
            matched = True
    ...
    if not matched:
        # Try with part1
        words_match = any(word in track_title_normalized 
                         for word in part1_normalized.split() if len(word) > 3)
        if words_match:
            score += 20
            matched = True
        ...
        if part2 and not matched:
            words_match = any(word in track_title_normalized 
                             for word in part2_normalized.split() if len(word) > 3)
            if words_match:
                score += 20
                matched = True
    ...
    if artist_name_normalized not in part1_normalized and artist_name_normalized not in part2_normalized:
        score -= 100
    ...
    bad_keywords = [' - ', 'ao vivo', 'live', 'remix', 'version', 'karaoke', 'acoustic', 
                    'instrumental', 'cover', 'radio edit', 'extended']
...
def get_all_songs():
   ...
   html = render_template_string(all_songs_template, pagination=pagination, table_lines=table_lines_html)
...
def get_songs_to_rename():
   ...
   table_lines_html = render_template_string(table_lines_template, songs=songs, skip=skip)
   html = render_template_string(songs_to_rename_template, table_lines=table_lines_html, page=page + 1, song_index=song_index )
...
def rename_song():
   ...
   if "new_name" in d and "old_name" in d:
       ...
       if k.is_song_in_queue(old_name):
           ...
           queue_error_msg = ({"success":False, "message":_("Error: Can't edit this song because it is in the current queue: ") + old_name, "categoryClass":"is-danger"})
       else:
           ...
           if os.path.isfile(os.path.join(k.download_path, new_name + file_extension)):
              ...
              queue_error_msg = ({"success":False, "message":_("Error renaming file: '%s' to '%s', Filename already exists")
                   % (old_name, new_name + file_extension), "categoryClass":"is-danger"})
           else:
       ...
       return jsonify({"success":False, "message":_("Error: No filename parameters were specified!"), "categoryClass":"is-danger"})     
