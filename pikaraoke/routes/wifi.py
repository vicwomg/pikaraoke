...
from pikaraoke.lib.get_platform import supports_wifi_editing

if supports_wifi_editing():
    import pywifi
    from pywifi.iface import Interface

    from pikaraoke.lib.setup_wifi import connect_to_wifi, get_all_wifi, get_current_wifi
...
@wifi_bp.route("/wifi")
def wifi_settings():
    if not supports_wifi_editing():
        flash(_("Wifi editing is not supported on this platform"), "is-danger")
...
def connect_wifi():
    if not supports_wifi_editing():
        return {"success": False, "message": _("WiFi editing is not supported on this platform")}
