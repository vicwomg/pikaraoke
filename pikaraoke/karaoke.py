...
class Karaoke:
    ...
    def __init__(
        ...
        self.screen = None
        self.youtubedl_version = self.read_youtubedl_version()
        ...
        if self.platform.is_rpi():
            ...
            while int(time.time()) < end_time:
                ...
                if not self.is_network_connected():
                    logger.debug("Couldn't get IP, retrying....")
                else:
                    break
        else:
            self.ip = self._get_ip()

        logger.debug("IP address (for QR code and splash screen): " + self.ip)

        if self.url_override != None:
            logger.debug("Overriding URL with " + self.url_override)
    ...
    def generate_qr_code(self):
        logger.debug("Generating URL QR code")
        qr = qrcode.QRCode(
            version=1,
            box_size=1,
            border=4,
        ...
        if result == 0:
            ...
            if enqueue:
                if found_song:
                    ...
                    logger.error("queueing song: " + video_url)
        else:
            logger.error("Error downloading song: " + video_url)

        return result

    def get_available_songs(self):
        logger.info(f"Fetching available songs in: {self.download_path}")
        ...
        for file in self.download_path.rglob("*.*"):
            if file.suffix.lower() in types and file.is_file():
                logger.debug("adding song: " + file.name)
    ...
    def rename(self, song_path: str, new_name: str):
        logger.info("Renaming song: '" + song_path + "' to: " + new_name)
    ...
    def find_song_by_youtube_id(self, youtube_id):
        for song in self.available_songs:
            if youtube_id in song:
                return song
        logger.error(f"No available song found with {youtube_id=}")
        return None
    ...
    def play_file(self, file: str, semitones: int = 0):
        logger.info(f"Playing file: {file} transposed {semitones} semitones")
        ...
        try:
            ...
            logger.error("Error resolving file: " + str(e))
            self.queue.pop(0)
            return False
        ...
        if fr.cdg_file_path != None:  # handle CDG files
            logger.info("Playing CDG/MP3 file: " + file)
        ...
        logger.debug(f"COMMAND: ffmpeg {args}")
        ...
        while self.ffmpeg_process.poll() is None:
            try:
                output = q.get_nowait()
                logger.debug("[FFMPEG] " + decode_ignore(output))
            except Empty:
                pass
            else:
    ...
    def start_song(self):
        logger.info(f"Song starting: {self.now_playing}")
        self.is_playing = True

    def end_song(self):
        logger.info(f"Song ending: {self.now_playing}")
        self.reset_now_playing()
        self.kill_ffmpeg()
        logger.debug("ffmpeg process killed")

    def transpose_current(self, semitones):
        logger.info(f"Transposing current song {self.now_playing} by {semitones} semitones")
    ...
    def enqueue(
        ...
        if self.is_song_in_queue(song_path):
            logger.warning(f"Song is already in queue, will not add: {song_path=}")
            return False
        else:
            queue_item = {
                "user": user,
            ...
            if add_to_front:
                logger.info(f"'{user}' is adding song to front of queue: {song_path}")
    ...
    def queue_add_random(self, amount):
        logger.info("Adding %d random songs to queue" % amount)
        ...
        while i < amount:
            ...
            if self.is_song_in_queue(songs[r]):
                logger.warning("Song already in queue, trying another... " + songs[r])
            else:
                self.enqueue(songs[r], "Randomizer")
                i += 1
            songs.pop(r)
            if len(songs) == 0:
                logger.warning("Ran out of songs!")
                return False
        return True
    ...
    def queue_edit(self, song_name, action):
        ...
        for each in self.queue:
            if song_name in each["file"]:
                ...
                index += 1
        if song == None:
            logger.error("Song not found in queue: " + song["file"])
            return False
        if action == "up":
            if index < 1:
                logger.warning("Song is up next, can't bump up in queue: " + song["file"])
                return False
            else:
                logger.info("Bumping song up in queue: " + song["file"])
                del self.queue[index]
                self.queue.insert(index - 1, song)
                return True
        elif action == "down":
            if index == len(self.queue) - 1:
                logger.warning("Song is already last, can't bump down in queue: " + song["file"])
                return False
            else:
                logger.info("Bumping song down in queue: " + song["file"])
                del self.queue[index]
                self.queue.insert(index + 1, song)
                return True
        elif action == "delete":
            logger.info("Deleting song from queue: " + song["file"])
            del self.queue[index]
            return True
        else:
            logger.error("Unrecognized direction: " + action)
            return False

    def skip(self):
        if self.is_file_playing():
            logger.info("Skipping: " + self.now_playing)
    ...
    def pause(self):
        if self.is_file_playing():
            logger.info("Toggling pause: " + self.now_playing)
    ...
    def vol_up(self):
        self.volume += 0.1
        logger.debug(f"Increasing volume by 10%: {self.volume}")
        if self.is_file_playing():
    ...
    def vol_down(self):
        ...
        if self.is_file_playing():
            ...
            logger.warning("Tried to volume down, but no file is playing!")
            return False
    ...
    def run(self):
        logger.info("Starting PiKaraoke!")
        logger.info(f"Connect the player host to: {self.url}/splash")
        self.running = True
        while self.running:
            try:
                ...
                logger.warning("Keyboard interrupt: Exiting pikaraoke...")
                self.running = False

    def read_youtubedl_version(self):
        self.youtubedl_version = check_output([YT_DLP_CMD, "--version"]).strip().decode("utf8")
        return self.youtubedl_version

    def upgrade_youtubedl(self):
        logger.info("Upgrading youtube-dl. Current version: %s" % self.youtubedl_version)
        try:
            output = (
                check_output([YT_DLP_CMD, "-U"], stderr=subprocess.STDOUT).decode("utf8").strip()
            )
            logger.info(output)
        except subprocess.CalledProcessError as error:
            output = error.output.decode("utf8")
            logger.error(f"Failed to upgrade youtubedl. {error=} {output=}")
