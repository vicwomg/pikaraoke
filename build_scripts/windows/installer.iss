...
; Logo path is relative to the PROJECT ROOT
...
[Files]
; --- UPDATED SECTION FOR ONEDIR BUILD ---
; We now grab everything inside the 'pikaraoke' folder using the wildcard (*)
; and the flags 'recursesubdirs createallsubdirs' to keep the folder structure.
Source: "..\..\dist\pikaraoke\*"; DestDir: "{app}"; Flags: ignoreversion recursesubdirs createallsubdirs; Components: main

; FFmpeg is still separate
...
Type: files; Name: "{userappdata}\pikaraoke\*.ini"; Check: ShouldRemoveData
Type: files; Name: "{userappdata}\pikaraoke\qrcode.png"; Check: ShouldRemoveData
Type: files; Name: "{userappdata}\pikaraoke\pikaraoke.db"; Check: ShouldRemoveData
Type: files; Name: "{userappdata}\pikaraoke\pikaraoke.log"; Check: ShouldRemoveData
Type: dirifempty; Name: "{userappdata}\pikaraoke"; Check: ShouldRemoveData
...
begin
  if MsgBox('Are you sure you want to uninstall PiKaraoke?' + #13#10 + #13#10 +
            'Your song library will be preserved.',
            mbConfirmation, MB_YESNO) = IDYES then
  begin
    Result := True;
    if MsgBox('Do you also want to delete your configuration, database, and log files?' + #13#10 +
              'Location: ' + ExpandConstant('{userappdata}\pikaraoke') + #13#10 + #13#10 +
              'Click YES to delete these files.' + #13#10 +
              'Click NO to keep them.',
              mbConfirmation, MB_YESNO) = IDYES then
    begin
      RemoveDataFiles := True;
    end
    else
    begin
